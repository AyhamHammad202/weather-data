      ...
      {chart && variable && (
        ...
        >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={goPrev}
            className={`absolute top-1/2 -translate-y-1/2 z-10 rounded-full flex items-center justify-center transition-all
              ${isMobile ? 'left-2 w-9 h-9 bg-black/60 border border-white/10' : 'left-4 w-12 h-12'}`}
            style={!isMobile ? { background: 'var(--bg-glass-heavy)', border: '1px solid var(--border-glass)', boxShadow: '0 0 20px var(--border-glass)' } : {}}
            aria-label="Previous chart"
          >
            <ChevronLeft size={isMobile ? 14 : 20} style={{ color: '#00e5ff' }} />
          </motion.button>
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b"
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  <span className="text-xs sm:text-sm font-medium" style={{ color: 'var(--text-primary)' }}>
                    {isMobile ? chart.monthShort : chart.monthName}
                  </span>
                </div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                >
                  <Star size={13} style={{ color: isFavorite ? '#ffd60a' : 'var(--text-muted)', fill: isFavorite ? '#ffd60a' : 'none' }} />
                </motion.button>
                ...
                >
                  <Info size={13} style={{ color: showInfo ? '#00e5ff' : 'var(--text-secondary)' }} />
                </motion.button>
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={onClose}
                  className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center ml-1 sm:ml-2"
                  style={{ background: 'rgba(255,23,68,0.1)', border: '1px solid rgba(255,23,68,0.3)' }}
                  aria-label="Close lightbox"
                >
                  <X size={13} style={{ color: '#ff1744' }} />
                </motion.button>
              </div>
            </div>
                    ...
                    <>
                      ...
                      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2 px-3 py-2 rounded-xl"
                        ...
                        <button onClick={() => zoomOut()} className="p-1 hover:text-neon-cyan transition-colors" aria-label="Zoom out">
                          <ZoomOut size={13} style={{ color: 'var(--text-secondary)' }} />
                        </button>
                        ...
                        <span className="text-[9px] ml-1 font-mono hidden sm:inline" style={{ color: 'var(--text-muted)' }}>
                      ...
                      >
                        ...
                        >
                          <img
                            ...
                            style={{ maxWidth: isMobile ? '95vw' : '80vw', maxHeight: isMobile ? 'calc(100vh - 140px)' : 'calc(100vh - 200px)', objectFit: 'contain', display: 'block' }}
                            draggable={false}
                          />
                        </motion.div>
                      </TransformComponent>
              ...
              <AnimatePresence>
                {showInfo && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0, width: isMobile ? 280 : 300 }}
                    exit={{ opacity: 0, x: 20, width: 0 }}
                    transition={{ duration: 0.3, ease: [0.4, 0, 0.2, 1] }}
                    className={`flex-shrink-0 overflow-hidden border-l h-full
                      ${isMobile ? 'absolute right-0 top-0 bottom-0 z-20 bg-glass-heavy backdrop-blur-md shadow-2xl' : 'relative'}`}
                    style={{ borderColor: 'var(--border-glass)' }}
                  >
                    <div className="w-72 h-full overflow-y-auto p-5 space-y-4 relative">
                      {isMobile && (
                        <button
                          onClick={() => setShowInfo(false)}
                          className="absolute right-4 top-4 p-1.5 rounded-lg hover:bg-white/5 text-neon-cyan transition-colors"
          ...
          <motion.button
            ...
            onClick={goNext}
            className={`absolute top-1/2 -translate-y-1/2 z-10 rounded-full flex items-center justify-center transition-all
              ${isMobile ? 'right-2 w-9 h-9 bg-black/60 border border-white/10' : 'right-4 w-12 h-12'}`}
            style={!isMobile ? { background: 'var(--bg-glass-heavy)', border: '1px solid var(--border-glass)', boxShadow: '0 0 20px var(--border-glass)' } : {}}
            aria-label="Next chart"
          >
            <ChevronRight size={isMobile ? 14 : 20} style={{ color: '#00e5ff' }} />
          </motion.button>
        </motion.div>
      )}
    </AnimatePresence>
